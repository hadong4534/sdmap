...
import { VendorListItem } from "@/components/ui";
...
const SHEETS = {
  cat: { title: "카테고리", opts: [["all","전체"],["studio","스튜디오"],["dress","드레스"],["makeup","메이크업"],["hall","웨딩홀"]] },
  region: { title: "지역", opts: REGIONS.map((r) => [r, r]) },
  price: { title: "가격대 (예상 최종가)", opts: PRICES },
  risk: { title: "추가금 위험", opts: RISKS },
  sort: { title: "정렬", opts: SORTS },
};
...
export default function Search() {
  ...
  useEffect(() => {
    ...
    if (c) setCat(c);
    if (supabase) supabase.from("vendors").select("*").eq("status", "active").then(({ data }) => setAll(data || []));
  }, []);
  ...
  list = [...list].sort((a, b) =>
    sort === "priceAsc" ? a.estimated_final_price - b.estimated_final_price :
    sort === "priceDesc" ? b.estimated_final_price - a.estimated_final_price :
    sort === "rating" ? b.rating - a.rating :
    sort === "riskAsc" ? a.risk_score - b.risk_score :
    b.review_count - a.review_count
  );

  const filterBtns = [["cat","카테고리"],["region","지역"],["price","가격"],["risk","추가금 위험"],["sort","정렬"]];
  const activeChips = [];
  if (cat !== "all") activeChips.push(["cat", CATS[cat]]);
  if (region !== "전체") activeChips.push(["region", region]);
  if (price !== "all") activeChips.push(["price", labelOf("price")]);
  if (risk !== "all") activeChips.push(["risk", labelOf("risk")]);
  if (sort !== "recommend") activeChips.push(["sort", labelOf("sort")]);

  return (
    <div className="min-h-screen bg-surface pb-20">
      <header className="sticky top-0 z-30 bg-surface/95 backdrop-blur border-b border-line">
        <div className="max-w-3xl mx-auto px-4 pt-3 pb-2">
          <div className="flex items-center gap-2 bg-white border border-line rounded-2xl px-3.5 h-11">
          ...
          <div className="flex gap-2 mt-2.5 overflow-x-auto no-scrollbar">
            {filterBtns.map(([k, l]) => {
              const on = val[k] !== (k === "region" ? "전체" : k === "sort" ? "recommend" : "all");
              return (
                <button key={k} onClick={() => setSheet(k)} className={`shrink-0 inline-flex items-center gap-1 px-3 h-8 rounded-full text-[12.5px] font-bold border ${on ? "bg-brand-500 text-white border-brand-500" : "bg-white text-body border-line"}`}>
                  {on ? labelOf(k) : l}
      ...
      <main className="max-w-3xl mx-auto px-4 py-3">
        ...
        <div className="space-y-3">
          {list.map((v) => <VendorListItem key={v.id} v={v} />)}
      ...
      {sheet && (
        <div className="fixed inset-0 z-50 flex items-end" onClick={() => setSheet(null)}>
          ...
          <div className="relative w-full bg-white rounded-t-3xl p-5 pb-8 max-h-[70vh] overflow-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-1">
              {SHEETS[sheet].opts.map(([v, l]) => (
                <button key={v} onClick={() => { setVal[sheet](v); setSheet(null); }} className={`w-full text-left px-4 h-12 rounded-xl text-[15px] font-bold ${val[sheet] === v ? "bg-brand-50 text-brand-700" : "text-body"}`}>{l}</button>
              ))}
            </div>
          </div>
        </div>
