...
const KakaoIcon = () => (
  <svg width="19" height="19" viewBox="0 0 24 24" fill="#000000" aria-hidden>
    <path d="M12 3C6.48 3 2 6.58 2 10.96c0 2.84 1.93 5.33 4.83 6.73-.21.74-.77 2.73-.88 3.15-.14.53.19.52.4.38.17-.11 2.68-1.82 3.76-2.56.6.09 1.23.13 1.89.13 5.52 0 10-3.58 10-7.96S17.52 3 12 3z"/>
  </svg>
);
const NaverIcon = () => (
  <svg width="15" height="15" viewBox="0 0 24 24" fill="#FFFFFF" aria-hidden>
    <path d="M16.27 12.84 7.95 1H1v22h6.73V11.16L16.05 23H23V1h-6.73z"/>
  </svg>
);
...
export default function Login() {
  ...
  async function oauth(provider) {
    if (provider === "naver") { setMsg("네이버 로그인은 키 등록 후 활성화됩니다."); return; }
    if (!isSupabaseReady || !supabase) { setMsg("Supabase 환경변수를 먼저 설정해 주세요."); return; }
  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center px-7 bg-surface">
      <div className="w-full max-w-sm flex flex-col items-center text-center">
        ...
        <img src="/images/logo_full.png" alt="스드맵" className="w-40 h-auto" />
        ...
        <div className="mt-7 w-full space-y-2.5">
          ...
          <div className="flex items-center gap-3 py-1.5 text-muted text-xs">
            <span className="flex-1 h-px bg-line" /> 또는 <span className="flex-1 h-px bg-line" />
          </div>

          <Link href="/signup?method=phone" className="w-full h-[52px] rounded-xl bg-brand-grad text-white font-extrabold text-[15px] flex items-center justify-center">
            휴대폰 번호로 가입
          </Link>
          <Link href="/signup?method=email" className="w-full h-[52px] rounded-xl bg-brand-50 text-brand-700 font-extrabold text-[15px] flex items-center justify-center">
            이메일(아이디)로 가입 · 로그인
          </Link>
          <Link href="/home" className="block text-muted text-[13px] font-bold pt-1">먼저 둘러보기</Link>
        </div>

        {msg && <p className="mt-4 text-[12px] text-brand-700 bg-brand-50 rounded-lg px-3 py-2">{msg}</p>}
        <p className="mt-5 text-[11px] text-muted leading-relaxed">가입 시 이용약관 및 개인정보 처리방침에 동의하는 것으로 간주됩니다.</p>
      </div>
    </main>
  );
}
